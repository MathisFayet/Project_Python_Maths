fitTab = [[0, 99, 90, 80, 70, 60, 50, 40, 30, 20, 10, 5, 3, 1],
...
def fitValidity(chi: int, degrees: int) -> str:
    index: int = 1
    found: bool = False

    while (index != 13):
        if fitTab[degrees][index] <= chi <= fitTab[degrees][index + 1]:
            max = fitTab[0][index]
            min = fitTab[0][index + 1]
            found = True
            break
        index += 1

    if not found:
        if (fitTab[degrees][1]) <= chi:
            max = fitTab[0][13]
            fitValidity = 'P < ' + str(max) + '%'
            return (fitValidity)

        if (fitTab[degrees][13] >= chi):
            max = fitTab[0][1]
            fitValidity = 'P > ' + str(max) + '%'
            return (fitValidity)

    fitValidity = str(min) + '% < P < ' + str(max)+'%'
    return (fitValidity)